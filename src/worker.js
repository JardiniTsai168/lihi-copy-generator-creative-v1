...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/generate-creative' && request.method === 'POST') {
      ...
      try {
        return await proxyBridgeRequest(request, env, endpointBase, '/api/generate-creative');
      } catch (err) {
        return new Response(JSON.stringify({
          ok: false,
          error: err.message,
...
function getCreativePlatformMeta(platform) {
  switch (platform) {
    case 'instagram':
      return { label: 'IG', width: 1440, height: 1800, sizeLabel: '1440 x 1800 4:5' };
    case 'threads':
      return { label: 'Threads', width: 1440, height: 2560, sizeLabel: '1440 x 2560 9:16' };
    case 'google_ads':
      return { label: 'Google Ads', width: 1200, height: 628, sizeLabel: '1200 x 628 1.91:1' };
    case 'facebook':
    default:
      return { label: 'Facebook 主圖', width: 1440, height: 1440, sizeLabel: '1440 x 1440 1:1' };
  }
}
